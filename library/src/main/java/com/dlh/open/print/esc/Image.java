...
import com.dlh.open.print.common.ImageConvert;
...
public class Image extends BaseESC {
    ...
    public Image(Port port, Printer.PrinterType printer_type) {
        super(port, printer_type);
    }

    /***
     * 图像数据下载到打印机内存  <p/>
     * 1)图像数据扫描方式是从左到右，从上到下 <br/>
     * 2)数据总大小:X_BYTES * Y_BYTES *8 <br/>
     * 3)X方向点数 X_BYTES * 8 <br/>
     * 4)Y方向点数 Y_BYTES * 8 <br/>
     * @param x_bytes
     * @param y_bytes
     * @param data
     * @return
    ...
    /***
     *  绘制图像到打印机绘图区域<p/>
     *  1)并不立刻打印输出,一些情况会导致打印内容输出<br/>
     *      A. 换行回车(\r \n 或者\r\n)<br/>
     *      B 打印对象导致x坐标超过画板宽度<br/>
     *  2)图像高度超过打印画板高度高度会导致上部分图像丢失<br/>
     * @param image_width_dots
     * @param image_height_dots
     * @param mode
     * @param image_data
     * @return
     */
