...
public class CardReader extends BaseESC {
     ...
     * 构造函数
     */
    public CardReader(Port port, Printer.PrinterType printer_type) {
    ...
    /// 1)请求的命令需要放在ADPU中
    /// 2)返回的结果在read中
    public boolean cardCPU_Read(int channel, char[] ADPU, int adpu_len, String from) {
        req[0] = (byte) channel;
        req[1] = 0;//read
    ...
    /// 2)返回的结果在ret中
    /// 3)Select 等语句用此方法
    public boolean cardCPU_Write(int channel, char[] ADPU, int adpu_len, String from) {
        req[0] = (byte) channel;
        req[1] = 1;//write
        for (int i = 0; i < adpu_len; i++) {
            req[2 + i] = (byte) ADPU[i];
